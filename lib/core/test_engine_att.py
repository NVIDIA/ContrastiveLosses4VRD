...
from core.test_att import im_detect_atts
from datasets_rel import task_evaluation_sg_att as task_evaluation_sg_att
from datasets_rel.json_dataset_att import JsonDatasetAtt
from modeling_rel import model_builder_att
import nn as mynn
from utils.detectron_weight_helper import load_detectron_weight
import utils.env as envu
import utils.net_rel as net_utils_rel
...
def test_net_on_dataset(
    ...
    dataset = JsonDatasetAtt(dataset_name)
    test_timer = Timer()
    test_timer.tic()
    if multi_gpu:
    ...
    task_evaluation_sg_att.eval_att_results(all_results, output_dir, args.do_val)
...
def multi_gpu_test_net_on_dataset(
    ...
    outputs = subprocess_utils.process_in_parallel(
        'att_detection', num_images, binary, output_dir,
    ...
    det_file = os.path.join(output_dir, 'att_detections.pkl')
    save_object(all_results, det_file)
    logger.info('Wrote att_detections to: {}'.format(os.path.abspath(det_file)))
...
def test_net(
    ...
    for i, entry in enumerate(roidb):
        ...
        im_results = im_detect_atts(model, im, dataset_name, box_proposals, timers)
        
        im_results.update(dict(image=entry['image']))
        # add gt
        if args.do_val:
            im_results.update(
                dict(gt_obj_boxes=entry['obj_boxes'],
                     gt_obj_labels=entry['obj_gt_classes'],
                     gt_att_labels=entry['att_gt_classes']))
        ...
        if i % 10 == 0:  # Reduce log file size
            ...
            det_time = (timers['im_detect_atts'].average_time)
            logger.info((
    ...
    if ind_range is not None:
        det_name = 'att_detection_range_%s_%s.pkl' % tuple(ind_range)
    else:
        det_name = 'att_detections.pkl'
...
def initialize_model_from_cfg(args, gpu_id=0):
    ...
    """
    model = model_builder_att.Generalized_RCNN()
    model.eval()
