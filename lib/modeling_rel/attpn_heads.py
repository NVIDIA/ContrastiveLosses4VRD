    ...
    def forward(self, det_rois, det_labels, det_scores, im_info, dataset_name, roidb=None):
        ...
        if self.training:
            ...
            if dataset_name.find('oi') >= 0:
                ...
                return_dict['obj_rois'] = det_rois
                return_dict['obj_labels'] = det_labels  # att_det_labels start from 1
                return_dict['obj_scores'] = det_scores
                return_dict['all_obj_labels_int32'] = det_labels.astype(np.int32, copy=False) - 1  # att_det_labels start from 1
                return_dict['fg_size'] = np.array([det_rois.shape[0]], dtype=np.int32)
